Anomaly_NT<-function(){
  ...
  ## 2. Line descriptions have note been kept in this particular model
  ...
  data.train<-data.train[-c(which.na(data.train[,11])),]
  data.test<-data[data$Period=="8/1/14"| data$Period=="9/1/14",]
  data.test$Period<-NULL
  data.test<-data.test[-c(which.na(data.test[,11])),]
  
  ## Selecting features manually
  data.train<-cbind(data.train[,c(1,3,4,5,11)],as.data.frame(rep("TRUE",nrow(data.train))))
  names(data.train)[6]<-"Flag"
  data.test<-data.test[,c(1,3,4,5,11)]
  
  ## Creating a one-class SVM model
  model<-ksvm(Flag~.,data=data.train,kernel=kernel,type="one-svc",nu=0.1)
  train.pred<-predict(model,data.train[,-6])
  ...
  names(data.test)[9:10]<-c("SVM Flag","Confidence")
  
  ## Creating final anomalous flags based on FALSE flags generated by both the methods
  data.test[,11]<-TRUE
  data.test[(data.test$`SVM Flag`==FALSE & data.test$`Distribution Flag`==FALSE),11]<-FALSE
  names(data.test)[11]<-"Final Flag"
